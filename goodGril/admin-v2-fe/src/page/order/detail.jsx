...
class OrderDetail extends React.Component{
	...
	//加载订单详情
	loadOrderDetail(){
			_order.getOrderDeatil(this.state.orderNumber).then((res)=>{
				this.setState({
					orderInfo:res
				});
			},(errMsg)=>{
				    ...
				    <div className="col-md-5">
				    	<p className="form-control-static">
				    		{this.state.orderInfo.statusDesc}
				    		{
				    			...
				    			?<button className="btn btn-defaul btn-sm btn-send-goods"
										...
										<tr key={index}>
											<td>
												<img className="p-img" alt={product.productName} 
													src={`${this.state.orderInfo.imgHost}${product.productImage}`} />
											</td>
